...
const { askQuestion, isRAGAvailable } = require('../services/insightops/ragEngine');
const { searchDocuments } = require('../services/insightops/vectorSearch');
...
router.post('/', asyncHandler(async (req, res) => {
    ...
    // Store chat history only if answer is valid
    let historyEntry = null;
    const entryId = uuidv4();

    if (result.answer && !result.answer.includes("[AI unavailable]")) {
        historyEntry = await ChatHistory.create({
            id: entryId,
            workspaceId,
    ...
    res.json({
        id: historyEntry ? historyEntry.id : entryId,
        question,
        ...result,
