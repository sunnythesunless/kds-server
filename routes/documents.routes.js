...
const { Document, DocumentVersion } = require('../models/insightops');
const asyncHandler = require('../middleware/asyncHandler');
const ApiError = require('../utils/ApiError');
const { getEmbedding } = require('../utils/insightops/vectorUtils');
...
router.get('/', asyncHandler(async (req, res) => {
    ...
    const docs = documents.rows.map(d => {
        const doc = d.toJSON();
...
router.post('/', asyncHandler(async (req, res) => {
    const { workspaceId, title, type, content } = req.body;

    // Use authenticated user's email as author
    const author = req.user.email || req.user.name || 'unknown';

    // Validate required fields
    if (!workspaceId || !title || !content) {
        throw new ApiError(400, 'Missing required fields: workspaceId, title, content');
    }
...
router.put('/:id', asyncHandler(async (req, res) => {
    ...
    const { title, type, content, changeNotes } = req.body;
    const author = req.user.email || req.user.name || 'unknown';
...
router.post('/:id/verify', asyncHandler(async (req, res) => {
    ...
    const verifiedBy = req.user.email || req.user.name || 'unknown';
